...
class instabot:
    ...
    def action_on_greedy_people(self, cheap_ppl):
        ...
        for k in cheap_ppl:
            ...
            # try:
            #     self.driver.find_element_by_xpath('//div[@class="aIYm8 coreSpriteSearchClear"]').click()
            # except:
            #     print('searchbox is already empty moving forward to search')
            # self.driver.find_element_by_xpath("//input[@placeholder='Search']").send_keys(k)
            # time.sleep(4)
            # mall = self.driver.find_element_by_xpath('//div[@class="fuqBx"]')
            # try:
            #     mall.find_element_by_xpath("//span[contains(text(), '{}')]".format(k)).click()
            # except:
            #     logging.warning('user {} does not exist please specify correct id'.format(k))
            #     pdb.set_trace()
            #     self.driver.find_element_by_xpath('//div[@class="aIYm8 coreSpriteSearchClear"]').click()
            #     #f.close()
            time.sleep(3)
